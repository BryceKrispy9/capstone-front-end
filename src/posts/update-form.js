...
export default class UpdateForm extends Component {
	...
	componentDidMount() {
		axios
			.get(
				`https://swim-coach-app-flask-api.herokuapp.com/fourm/${this.state.id}`
			)
			.then((response) => {
				this.setState({
					fourm_title: response.data.fourm_title,
					intensity_content: response.data.intensity_content,
					focus_content: response.data.focus_content,
					equipment_content: response.data.equipment_content,
					yardage_content: response.data.yardage_content,
				});
			})
			.catch((error) => {
	...
	handleSubmit(event) {
		axios({
			method: "put",
			url: `https://swim-coach-app-flask-api.herokuapp.com/fourm/${this.state.id}`,
			data: this.buildPost(),
		})
			.then((response) => {
			...
			<form onSubmit={this.handleSubmit} className="post-form-wrapper">
				...
				<div className="one-column">
					<RichTextEditor
						...
						contentToEdit={
							this.stateintensity_content &&
							this.state.focus_content &&
							this.state.equipment_content &&
							this.state.yardage_content
							// ? this.props.intensity_content &&
							//   this.props.post.focus_content &&
							//   this.props.equipment_content &&
							//   this.props.yardage_content
							// : null
						}
					/>
				</div>
